...
// Deterministic helpers: identical inputs produce identical results.
...
// Straw2-style score; the greatest valid-weight score wins.
...
// Equivalent score using a precomputed seed/stream mix.
inline double deterministicWeightedScorePremixed(uint64_t premixed_stream,
                                                 uint64_t key, double weight) {
    if (!(weight > 0.0) || !std::isfinite(weight)) {
        return -std::numeric_limits<double>::infinity();
    }
    const uint64_t hash = deterministicRandomHash(premixed_stream, key);
    constexpr double kTwoTo53 = 9007199254740992.0;
    const double uniform = static_cast<double>((hash >> 11) + 1) / kTwoTo53;
    return std::log(uniform) / weight;
}
