...
class BlockingAllocateOffsetBufferAllocator : public OffsetBufferAllocator {
   public:
    BlockingAllocateOffsetBufferAllocator(std::string segment_name, size_t base,
                                          size_t size,
...
class SegmentTest : public ::testing::Test {
   protected:
    static std::shared_mutex& AccessNoFSegmentMutex(
        NoFSegmentManager& manager) {
        return manager.segment_mutex_;
    }
...
TEST_F(SegmentTest, DetachedAllocationDoesNotBlockLivenessTransition) {
    ...
    auto transition = std::async(std::launch::async, [&] {
        return client_liveness_->Evaluate(ClientLivenessRecord::Clock::now(),
...
TEST_F(SegmentTest, SharedNameRegistrationsSurviveSegmentSnapshotRestore) {
...

    {
        ...
        ASSERT_EQ(
            segment_access.MountSegment(first, first_owner, client_liveness_),
            ErrorCode::OK);
...
TEST_F(SegmentTest, NoFSpaceReportsTrackMountedSegments) {
    ...
    for (const auto& report : reports) {
        ...
        EXPECT_EQ(report.capacity, 16 * 1024 * 1024U);
...
TEST_F(SegmentTest, NoFSpaceReportsRunConcurrentlyWithSharedLockHolders) {
    NoFSegmentManager manager(BufferAllocatorType::OFFSET);
    const UUID client = generate_uuid();
    auto segment = MakePtNoFSegment(0, "nof-blocking", "host-a");
    {
        auto access = manager.getNoFSegmentAccess();
    ...
    std::shared_lock<std::shared_mutex> held(AccessNoFSegmentMutex(manager));
